        ...
        function scrollToPos(time, index, manual, dir, active) {
            ...
            if (trigger('validate', [$markup, index, time, dir]) !== false) {
                ...
                $('.dw-ul', $markup).each(function (i) {
					...
					var thisValArr = $($elm).val().split(".");
					if($($elm).attr("class").indexOf("dwtd") < 0){
						if(i == 1){
							var res = getValid(thisValArr[i] - 1, t, dir, multiple, true);
						}else{
							var res = getValid(thisValArr[i], t, dir, multiple, true);
						}
					}else{
						var res = getValid(that._tempWheelArray[i], t, dir, multiple, true);
					}
